...
function hasNativeEditContext() {
    return typeof globalThis.EditContext === 'function';
}
/**
 ...
 */
function suppressXtermInput(container, textarea) {
    const stop = (ev) => {
    ...
    const events = ['input', 'compositionstart', 'compositionupdate', 'compositionend'];
...
export class EditContextAddon {
     ...
     */
    updateBounds(controlBounds, selectionBounds) {
        const ec = this._editContext;
        if (!ec)
            return;
    ...
    // ── Setup ───────────────────────────────────────────────────────────
    async _setup(terminal, container) {
    ...
    async _createEditContext(element) {
        if (hasNativeEditContext()) {
            const ec = new globalThis.EditContext();
     ...
     */
    _computeInput(oldText, rangeStart, rangeEnd, newText) {
    ...
    _wireCharacterBounds(ec, terminal) {
        const handler = (ev) => {
            ...
            for (let i = e.rangeStart; i < e.rangeEnd; i++) {
                ...
                bounds.push(new DOMRect(rect.left + col * cellW, rect.top + row * cellH, cellW, cellH));
            }
    ...
    _wireKeyboard(overlay, terminal) {
        const textarea = terminal.textarea;
        if (!textarea)
            return;
    ...
    _isTextInputKey(ev) {
        ...
        if (ev.key === 'Process' || ev.keyCode === 229)
            return true;
    ...
    _wireFocus(overlay, terminal, container) {
        ...
        const onTextareaBlur = (ev) => {
            if (!ev.isTrusted)
                return; // our synthetic event; let xterm see it
            if (redirecting) {
                ev.stopImmediatePropagation();
            }
        };
...
//# sourceMappingURL=EditContextAddon.js.map